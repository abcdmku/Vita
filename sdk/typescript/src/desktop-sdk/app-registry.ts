...
function freezeLaunchableApp(app: DesktopLaunchableApp): DesktopLaunchableApp {
  switch (app.surfaceKind) {
    case "tsx": {
      ...
      if (app.runtime.props !== undefined) {
        runtime.props = deepFreezeJson(app.runtime.props) as NonNullable<TsxComponentRef["props"]>;
      }
...
// Deep-clone a PlainJson value into fresh, deeply-frozen plain JSON. The source
// (e.g. a caller-supplied `runtime.props` object) is copied by VALUE so that
// mutating the original after the registry is built cannot reach the frozen
// registry output. `props` is typed `PlainJsonObject` (pure JSON — no functions,
// dates, or cycles), so a structural recursion is sufficient and total.
function deepFreezeJson(value: PlainJson): PlainJson {
  if (Array.isArray(value)) {
    const cloned: PlainJson[] = [];

    for (let index = 0; index < value.length; index += 1) {
      cloned.push(deepFreezeJson(value[index] as PlainJson));
    }
  ...
  if (value !== null && typeof value === "object") {
    const cloned: Record<string, PlainJson> = {};

    for (const key of Object.keys(value)) {
      cloned[key] = deepFreezeJson((value as Record<string, PlainJson>)[key] as PlainJson);
    }
