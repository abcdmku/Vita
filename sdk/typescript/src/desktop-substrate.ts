...
const HEARTBEAT_CHANNEL_FIELDS = Object.freeze(["fileName", "nonceFileName", "volumeName"]);
...
function readHeartbeatChannel(
  ...
  const volumeName = readRequiredString(child, "volumeName", [...path, "volumeName"], errors);
  const fileName = readRequiredString(child, "fileName", [...path, "fileName"], errors);
  const nonceFileName = readRequiredString(child, "nonceFileName", [...path, "nonceFileName"], errors);

  if (volumeName !== undefined && !validVolumeName(volumeName)) {
    addError(errors, [...path, "volumeName"], "Expected safe desktop heartbeat volume name.");
  }
  if (fileName !== undefined && !validHeartbeatFileName(fileName)) {
    addError(errors, [...path, "fileName"], "Expected safe desktop heartbeat file name.");
  }
  if (nonceFileName !== undefined && !validHeartbeatFileName(nonceFileName)) {
    addError(errors, [...path, "nonceFileName"], "Expected safe desktop heartbeat nonce file name.");
  }
  if (
    fileName !== undefined &&
    nonceFileName !== undefined &&
    fileName === nonceFileName
  ) {
    addError(errors, [...path, "nonceFileName"], "Heartbeat nonce file must be distinct.");
  }
  if (
    volumeName === undefined ||
    fileName === undefined ||
    nonceFileName === undefined ||
    !validVolumeName(volumeName) ||
    !validHeartbeatFileName(fileName) ||
    !validHeartbeatFileName(nonceFileName) ||
    fileName === nonceFileName
  ) {
    return undefined;
  }
...
function validVolumeName(value: string): boolean {
  return /^[a-z0-9][a-z0-9._-]{0,127}$/u.test(value);
}

function validHeartbeatFileName(value: string): boolean {
  return /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(value);
}
...
function parseDesktopHeartbeatLine(
  line: string,
): DesktopSubstrateValidationResult<DesktopSessionHeartbeat> {
  ...
  if (fields === undefined) {
    return reject([
      Object.freeze({
        message: "Expected desktop heartbeat marker.",
        path: "",
      }),
    ]);
  }
