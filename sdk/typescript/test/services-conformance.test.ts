...
const MANIFEST_PATH = resolve(REPO_ROOT, "schema", "capabilities", "services.json");
...
const SERVICES_MANIFEST = readServicesManifest();
const validateServices = compileCapabilityValidator(SERVICES_MANIFEST);
...
test("services conformance corpus matches the TS validator", () => {
  const vectors = readConformanceCorpus();

  assert.ok(vectors.length >= 15, "services conformance corpus must have at least 15 vectors");
  ...
  for (let index = 0; index < vectors.length; index += 1) {
    ...
    const result = validateServices(parsed.value);
...
function readServicesManifest(): CapabilityManifest {
  const raw = JSON.parse(readFileSync(MANIFEST_PATH, "utf8")) as unknown;
  const loaded = loadCapabilityManifest(raw);

  if (!loaded.ok) {
    assert.fail(`services manifest failed to load: ${loaded.reason}`);
  }
...
function readConformanceCorpus(): readonly ConformanceVector[] {
  ...
  if (!Array.isArray(raw)) {
    assert.fail("services conformance corpus must be an array");
  }
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (!isRecord(item)) {
      assert.fail(`services conformance vector ${index} must be an object`);
    }
