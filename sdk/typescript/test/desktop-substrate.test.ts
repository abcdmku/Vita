...
test("desktop stub manifest is a separable desktop package over capsule install launch stop", () => {
  ...
  assert.deepEqual(manifest.session.heartbeatChannel, {
    fileName: "heartbeat.line",
    nonceFileName: "launch-nonce",
    volumeName: "desktop-session",
  });
...
test("desktop session heartbeat is read through the substrate channel with exact nonce", async () => {
  const result = runDesktopStubSession({
    emit: () => {},
    nonce: () => "stub-test-nonce",
    now: () => "2026-06-24T00:00:00.000Z",
  ...
  const volumePath = "/var/lib/vita/runtime/volumes/com.vita.desktop.stub/desktop-session";
  const read = await readDesktopSessionHeartbeatThroughSubstrate({
    expectedNonce: "stub-test-nonce",
    launchedAfterMs: Date.parse("2026-06-23T23:59:59.000Z"),
    readTextFile: async (path) => {
      assert.equal(path, `${volumePath}/heartbeat.line`);
      return `${result.heartbeatLine}\n`;
    },
    registration,
    volumePath,
  });
  ...
  assert.equal(read.line, result.heartbeatLine);
  assert.equal(read.heartbeat.nonce, "stub-test-nonce");
  assert.equal(
    desktopSessionHeartbeatPath(volumePath, registration.heartbeatChannel),
    `${volumePath}/heartbeat.line`,
  );
  assert.equal(
    desktopSessionLaunchNoncePath(volumePath, registration.heartbeatChannel),
    `${volumePath}/launch-nonce`,
  );

  const stale = await readDesktopSessionHeartbeatThroughSubstrate({
    expectedNonce: "different-nonce",
    launchedAfterMs: Date.parse("2026-06-23T23:59:59.000Z"),
    readTextFile: async () => `${result.heartbeatLine}\n`,
    registration,
    volumePath,
  });
