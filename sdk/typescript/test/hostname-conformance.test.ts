...
const validateHostname = compileCapabilityValidator(HOSTNAME_MANIFEST);
...
test("hostname conformance corpus matches the TS validator", () => {
  const vectors = readConformanceCorpus();

  assert.ok(vectors.length >= 15, "hostname conformance corpus must have at least 15 vectors");
  ...
  for (let index = 0; index < vectors.length; index += 1) {
    ...
    const result = validateHostname(vector.request);
...
function readConformanceCorpus(): readonly ConformanceVector[] {
  ...
  if (!Array.isArray(raw)) {
    assert.fail("hostname conformance corpus must be an array");
  }
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (!isRecord(item)) {
      assert.fail(`hostname conformance vector ${index} must be an object`);
    }
