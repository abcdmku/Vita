...
const MANIFEST_PATH = resolve(REPO_ROOT, "schema", "capabilities", "capsule.json");
...
const CAPSULE_MANIFEST = readCapsuleManifest();
const validateCapsule = compileCapabilityValidator(CAPSULE_MANIFEST);
...
test("capsule conformance corpus matches the TS validator", () => {
  const vectors = readConformanceCorpus();

  assert.ok(vectors.length >= 25, "capsule conformance corpus must have at least 25 vectors");
  ...
  for (let index = 0; index < vectors.length; index += 1) {
    ...
    const result = validateCapsule(vector.request);
...
function readCapsuleManifest(): CapabilityManifest {
  const raw = JSON.parse(readFileSync(MANIFEST_PATH, "utf8")) as unknown;
  const loaded = loadCapabilityManifest(raw);

  if (!loaded.ok) {
    assert.fail(`capsule manifest failed to load: ${loaded.reason}`);
  }
...
function readConformanceCorpus(): readonly ConformanceVector[] {
  ...
  if (!Array.isArray(raw)) {
    assert.fail("capsule conformance corpus must be an array");
  }
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (!isRecord(item)) {
      assert.fail(`capsule conformance vector ${index} must be an object`);
    }
