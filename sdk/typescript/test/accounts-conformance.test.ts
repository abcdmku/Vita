...
const MANIFEST_PATH = resolve(REPO_ROOT, "schema", "capabilities", "accounts.json");
...
const ACCOUNTS_MANIFEST = readAccountsManifest();
const validateAccounts = compileCapabilityValidator(ACCOUNTS_MANIFEST);
...
test("accounts conformance corpus matches the TS validator", () => {
  const vectors = readConformanceCorpus();

  assert.ok(vectors.length >= 28, "accounts conformance corpus must have at least 28 vectors");
  ...
  for (let index = 0; index < vectors.length; index += 1) {
    ...
    const result = validateAccounts(vector.request);
...
function readAccountsManifest(): CapabilityManifest {
  const raw = JSON.parse(readFileSync(MANIFEST_PATH, "utf8")) as unknown;
  const loaded = loadCapabilityManifest(raw);

  if (!loaded.ok) {
    assert.fail(`accounts manifest failed to load: ${loaded.reason}`);
  }
...
function readConformanceCorpus(): readonly ConformanceVector[] {
  ...
  if (!Array.isArray(raw)) {
    assert.fail("accounts conformance corpus must be an array");
  }
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (!isRecord(item)) {
      assert.fail(`accounts conformance vector ${index} must be an object`);
    }
