...
const validateTimeSet = compileCapabilityValidator(TIME_MANIFEST);
...
test("time.set conformance corpus matches the TS validator", () => {
  const vectors = readConformanceCorpus();

  assert.ok(vectors.length >= 20, "time.set conformance corpus must have at least 20 vectors");
  ...
  for (let index = 0; index < vectors.length; index += 1) {
    ...
    const result = validateTimeSet(vector.request);
...
function readConformanceCorpus(): readonly ConformanceVector[] {
  ...
  if (!Array.isArray(raw)) {
    assert.fail("time.set conformance corpus must be an array");
  }
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (!isRecord(item)) {
      assert.fail(`time.set conformance vector ${index} must be an object`);
    }
