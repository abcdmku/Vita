...
boot_ts() {
  ...
  kill "$qpid" 2>/dev/null; pkill -f qemu-system-x86_64 >/dev/null 2>&1
  local ts ev pv ex cn st ac ar cpv cc cr pds pw cx cxr fe vo
  ...
  fe=$(grep -aE 'VITA-CAPSULE-FETCH:.*verified=OK' "$log" | tail -1); vo=$(grep -aE 'VITA-CAPSULE-VOLUME:.*mounted=OK' "$log" | tail -1)
  echo "----- markers -----"; for m in "$ts" "$ev" "$pv" "$ex" "$cn" "$st" "$ac" "$ar" "$cpv" "$cc" "$cr" "$pds" "$pw" "$cx" "$cxr" "$fe" "$vo"; do echo "  $m"; done
