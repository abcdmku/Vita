...
boot_ts() {
  ...
  kill "$qpid" 2>/dev/null; pkill -f qemu-system-x86_64 >/dev/null 2>&1
  local ts ev pv ex cn st ac ar cpv cc cr pds pw cx cxr fe hl vo of oe ol np we nn eg ig fl pr dr bk vu pd rf xp sh lc fs ow pq rl rlr dp
  ...
  we=$(grep -aE 'VITA-CAPSULE-WASM-EXECUTED:.*health=OK' "$log" | tail -1)
  nn=$(grep -aE 'VITA-CAPSULE-NET-NS:.*loopback=OK.*isolation=enforced' "$log" | tail -1)
  eg=$(grep -aE 'VITA-CAPSULE-NET-EGRESS:.*reach=OK.*drop=enforced' "$log" | tail -1)
  ig=$(grep -aE 'VITA-CAPSULE-NET-INGRESS:.*reach=OK.*drop=enforced' "$log" | tail -1)
  fl=$(grep -aE 'VITA-FILES:.*roundtrip=OK' "$log" | tail -1)
  pr=$(grep -aE 'VITA-PDS-REPO:.*status=OK' "$log" | tail -1)
  dr=$(grep -aE 'VITA-DRIFT:.*status=OK' "$log" | tail -1)
  bk=$(grep -aE 'VITA-BACKUP:.*verified=OK' "$log" | tail -1)
  ...
  lc=$(grep -aE 'VITA-CAPSULE-LIFECYCLE: update=OK rollback-on-unhealthy=OK' "$log" | tail -1)
  fs=$(grep -aE 'VITA-FILES-SHARED:.*role=owner.*roundtrip=OK status=OK' "$log" | tail -1)
  ow=$(grep -aE 'VITA-OWNER: challenge=issued reject=enforced status=OK' "$log" | tail -1)
  pq=$(grep -aE 'VITA-PDS-QUERY:.*records=[0-9]+ status=OK' "$log" | tail -1)
  rl=$(grep -aE 'VITA-ROLES: roles=6 enforced=OK.*roundtrip=OK status=OK' "$log" | tail -1)
  rlr=$(grep -aE 'VITA-ROLES-REJECT: reason=role_forbidden status=OK' "$log" | tail -1)
  dp=$(grep -aE 'VITA-DESKTOP-PKG: class=desktop installed=OK launched=OK heartbeat=OK headless-boundary=OK status=OK' "$log" | tail -1)
  echo "----- markers -----"; for m in "$ts" "$ev" "$pv" "$ex" "$cn" "$st" "$ac" "$ar" "$cpv" "$cc" "$cr" "$pds" "$pw" "$cx" "$cxr" "$fe" "$hl" "$vo" "$of" "$oe" "$ol" "$np" "$we" "$nn" "$eg" "$ig" "$fl" "$pr" "$dr" "$bk" "$vu" "$pd" "$rf" "$xp" "$sh" "$lc" "$fs" "$ow" "$pq" "$rl" "$rlr" "$dp"; do echo "  $m"; done
  ...
  if [ "$ok" = 1 ] && [ -n "$ts" ] && [ -n "$ev" ] && [ -n "$pv" ] && [ -n "$ex" ] && [ -n "$st" ] && [ -n "$ac" ] && [ -n "$ar" ] && [ -n "$cpv" ] && [ -n "$cc" ] && [ -n "$cr" ] && [ -n "$pds" ] && [ -n "$pw" ] && [ -n "$cx" ] && [ -n "$cxr" ] && [ -n "$fe" ] && [ -n "$hl" ] && [ -n "$vo" ] && [ -n "$of" ] && [ -n "$oe" ] && [ -n "$ol" ] && [ -n "$np" ] && [ -n "$we" ] && [ -n "$nn" ] && [ -n "$eg" ] && [ -n "$ig" ] && [ -n "$fl" ] && [ -n "$pr" ] && [ -n "$dr" ] && [ -n "$bk" ] && [ -n "$vu" ] && [ -n "$pd" ] && [ -n "$rf" ] && [ -n "$xp" ] && [ -n "$sh" ] && [ -n "$lc" ] && [ -n "$fs" ] && [ -n "$ow" ] && [ -n "$pq" ] && [ -n "$rl" ] && [ -n "$rlr" ] && [ -n "$dp" ]; then
    echo "RESULT: PASS (full control-plane: fetch+execute+health+volume+OCI(fetch/run/limits)+NET-PARSE+NET-NS+NET-EGRESS+NET-INGRESS + Phase4/5: FILES(roundtrip)+PDS-REPO+DRIFT+BACKUP(measured restore)+CONTROLLER-UI(VITA-UI)+PROTECT-DASH(measured)+RESTORE-FULL(measured re-hash)+EXPORT(verifiable)+STORAGE-HEALTH(measured)+LIFECYCLE(update+rollback)+SHARED-FOLDERS(owner measured;member-forbidden deferred)+OWNER(passkey verify-only fail-closed)+PDS-QUERY(measured)+ROLES(closed six-role set measured from agentd GET /roles; role_forbidden measured by omission)+DESKTOP-PKG(separable desktop pkg installs/launches/stops measured; heartbeat via substrate seam w/ exact nonce; headless-boundary live re-checked with desktop present) — wave5 OCI + wave6 net S1-S4 + 22 features)"
  else
