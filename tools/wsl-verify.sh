...
boot_ts() {
  ...
  kill "$qpid" 2>/dev/null; pkill -f qemu-system-x86_64 >/dev/null 2>&1
  local ts ev pv ex cn st ac ar cpv cc cr pds pw cx cxr fe hl vo of oe ol np we nn eg ig fl pr dr bk vu pd rf xp sh lc fs
  ...
  echo "----- markers -----"; for m in "$ts" "$ev" "$pv" "$ex" "$cn" "$st" "$ac" "$ar" "$cpv" "$cc" "$cr" "$pds" "$pw" "$cx" "$cxr" "$fe" "$hl" "$vo" "$of" "$oe" "$ol" "$np" "$we" "$nn" "$eg" "$ig" "$fl" "$pr" "$dr" "$bk" "$vu" "$pd" "$rf" "$xp" "$sh" "$lc" "$fs"; do echo "  $m"; done
