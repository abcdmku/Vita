...
boot_ts() {
  ...
  kill "$qpid" 2>/dev/null; pkill -f qemu-system-x86_64 >/dev/null 2>&1
  local ts ev pv ex cn st ac ar cpv cc cr pds pw cx cxr fe hl vo of
  ...
  echo "----- markers -----"; for m in "$ts" "$ev" "$pv" "$ex" "$cn" "$st" "$ac" "$ar" "$cpv" "$cc" "$cr" "$pds" "$pw" "$cx" "$cxr" "$fe" "$hl" "$vo" "$of"; do echo "  $m"; done
  ...
  if [ "$ok" = 1 ] && [ -n "$ts" ] && [ -n "$ev" ] && [ -n "$pv" ] && [ -n "$ex" ] && [ -n "$st" ] && [ -n "$ac" ] && [ -n "$ar" ] && [ -n "$cpv" ] && [ -n "$cc" ] && [ -n "$cr" ] && [ -n "$pds" ] && [ -n "$pw" ] && [ -n "$cx" ] && [ -n "$cxr" ] && [ -n "$fe" ] && [ -n "$hl" ] && [ -n "$vo" ] && [ -n "$of" ]; then
    echo "RESULT: PASS (full control-plane on-device: fetch(SRI)+capsule.execute(hardened)+health+persistent-volume+OCI-image-fetch(digest-verified) — wave 4 complete, wave 5 OCI fetch landed)"
  else
