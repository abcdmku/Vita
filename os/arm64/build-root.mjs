...
import {
  DEFAULT_DEBIAN_SNAPSHOT,
  DEFAULT_PACKAGE_ALLOWLIST,
  DEFAULT_SOURCE_DATE_EPOCH,
  PINNED_MKOSI_IMAGE,
} from "../x86_64/build-root.mjs";

export { DEFAULT_DEBIAN_SNAPSHOT, DEFAULT_SOURCE_DATE_EPOCH, PINNED_MKOSI_IMAGE };

export const DEFAULT_PACKAGE_ALLOWLIST_ARM64 = Object.freeze(
  DEFAULT_PACKAGE_ALLOWLIST.flatMap((packageName) =>
    packageName === "linux-image-amd64" ? ["linux-image-arm64", "raspi-firmware"] : [packageName],
  ),
);
...
export const DEFAULT_VITA_OVERLAYS_ARM64 = deepFreeze([
  ...
  {
    name: "agent-overlay",
    kind: "privileged-agent-overlay",
    relativePath: "os/arm64/agent-overlay",
    containerPath: "/work/os/arm64/agent-overlay",
    configPath: "os/arm64/agent-image.conf",
    requiredPaths: [
      "/usr/lib/vita/agentd",
      "/usr/bin/vita-agentd",
...
Distribution=debian
Release=trixie
Mirror=https://snapshot.debian.org/archive/debian/${DEFAULT_DEBIAN_SNAPSHOT}/
Repositories=main
...
const ARM64_CONFIG_TEXT = `[Include]
...
[Output]
Output=vita-debian-trixie-arm64-root

[Content]
# arm64 repeats the full runtime package allowlist so the guest carries
# cryptsetup-bin for later Vita storage slices, swaps only the kernel package,
# and adds Raspberry Pi 5 firmware staging.
# Keep in lockstep with build-root.mjs and test/root-determinism.test.ts.
Packages=
    bash
    ca-certificates
    coreutils
...
const DEFAULT_ROOT_BUILD_INPUT = Object.freeze({
  ...
  archConfigText: ARM64_CONFIG_TEXT,
  workspacePath: "/workspace/vita",
  outputPath: "/workspace/vita/os/arm64/out",
});

const COMMON_CONFIG_PATH = "os/common/mkosi.conf";
const ARCH_CONFIG_PATH = "os/arm64/mkosi.conf";
const COMMON_CONTAINER_PATH = "/work/os/common/mkosi.conf";
const ARCH_CONTAINER_PATH = "/work/os/arm64/mkosi.conf";
const ARCH_DIRECTORY = "/work/os/arm64";
...
export function planRootBuild(input = DEFAULT_ROOT_BUILD_INPUT) {
  ...
  const overlayArgs = DEFAULT_VITA_OVERLAYS_ARM64.map((overlay) => `--extra-tree=${overlay.containerPath}`);
  const dockerArgs = [
    "run",
    "--rm",
    "--pull=never",
  ...
  return deepFreeze({
    ...
    overlays: {
      ...
      required: true,
      extraTrees: DEFAULT_VITA_OVERLAYS_ARM64,
    },
...
function resolveMkosiConfig(commonConfig, archConfig) {
  ...
  assertEquals(
    mirror,
    `https://snapshot.debian.org/archive/debian/${DEFAULT_DEBIAN_SNAPSHOT}/`,
    "Distribution.Mirror",
  );
  assertArrayEquals(repositories, ["main"], "Distribution.Repositories");
  assertEquals(architecture, "arm64", "Distribution.Architecture");
  assertEquals(outputFormat, "directory", "Output.Format");
  assertEquals(imageVersion, "0.1-bootstrap", "Output.ImageVersion");
  assertEquals(output, "vita-debian-trixie-arm64-root", "Output.Output");
  assertArrayEquals(packages, DEFAULT_PACKAGE_ALLOWLIST_ARM64, "Content.Packages");
...
function readRootBuildInput(repoRoot) {
  return {
    ...
    outputPath: join(repoRoot, "os", "arm64", "out"),
  };
}

function printUsage() {
  console.error("usage: node os/arm64/build-root.mjs [--print-plan]");
}
