...
- **A contract RE-SCOPE must go in the main body (Objective/acceptance), NOT a `## Reviewer revision` section.** `tools/dispatch/review-task.mjs` STRIPS `## Reviewer revision` sections before showing the contract to the reviewer (the fix for the P3-001 false-negative). That is right for incremental *patch* instructions (the worker applies them, the reviewer judges the code), but WRONG for a *re-scope* that changes what the reviewer should check: P9-002's round-4 re-scope (hostname-only, IP deferred) lived in a revision section, so the reviewer never saw it and (correctly, against the stale Objective) flagged the implementation as deviation + "weakened tests". Round 5 moved the scope into the Objective + acceptance → approved. Rule: patches → revision section; scope changes → edit the body.
